...
class BorrowingSerializerTest(TestCase):
    ...
    def test_borrowing_serializer_fields(self):
        ...
        serializer = BorrowingSerializer(self.borrowing, context={'request': self.request})
        data = serializer.data
        
        self.assertEqual(set(data.keys()), {'id', 'book', 'expected_return_date', 'payments'})
        self.assertEqual(data['book'], self.book.id)
        self.assertEqual(data['expected_return_date'], self.borrowing.expected_return_date.isoformat())
    ...
    def test_borrowing_serializer_create_without_request(self):
        ...
        self.assertIn('Request context is required for payment creation', str(context.exception))
